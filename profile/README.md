...
layer. Every repo in this org is Apache-2.0. Fork the org shape, point its
environment values — project id, domain, registry paths — at your own in the
Terraform variables, the platform's environment file, its image paths and the
tenant files, and you have the floor of a software factory of your own.
...
The Claims column follows the register's assignment. The State column says
where a claim was actually graded when that differs: C-01 and C-03 (M1
claims) were graded at M2's close, and C-08 (an M2 stretch) is deferred to M3.
...
- **Recreate the org shape, not just the code.** The repos above, two teams
  (`platform`, `security`), a `main` ruleset on every repo once it has a
  first commit (PRs required, no force-push), CODEOWNERS routing
  `edge-config/external/` to security. This reference org requires zero
  approvals, so CODEOWNERS routes review without enforcing it
  ([ADR-0018](https://github.com/platform-factory/platform-factory-concept/blob/main/docs/adr/0018-engine-permissions-are-a-list-in-platform-roles.md),
  decision 7). The M1 build log records this scaffold taking under two hours.
...
- **`platform-config`** is pointed at by `3-argocd`'s root Application — fork
  it, change the repo URL in that layer's values, and change the project id
  in the files its README lists. Pinned versions carry their verification
  dates; bumping one is a pull request like any other change.
